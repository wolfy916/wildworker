...
public class SystemController {
    ...
    public void getUserLocation(@Header("simpSessionId") String sessionId,
        ...
        log.info("ws /location");

        var user = headerAccessor.getUser();
        log.info("-- user: {}", user);

//        SessionUser sessionUser = ((PrincipalDetails) ((OAuth2AuthenticationToken) token).getPrincipal()).getSessionUser();
//        log.info("-- principal: {}", token);
//        log.info("-- details: {}", sessionUser);

//        log.info("/system/location: {}", user.getName());
