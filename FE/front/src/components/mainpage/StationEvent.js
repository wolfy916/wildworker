...
function StationEvent(props) {
  ...
  const stationEventObj = {
    역삼역: [ssafyMan1, ssafyMan2, ssafyWoman1, ssafyWoman2],
    신도림역: [businessMan1, businessMan2, businessWoman],
    사당역: [businessMan1, businessMan2, businessWoman],
    잠실역: [lotteWoman, lotteMan],
  };
  ...
  React.useEffect(() => {
    ...
    function humanObjectAppear() {
      ...
      const randomValue =
        stationEventObj[stationName][
          Math.floor(Math.random() * stationEventObj[stationName].length)
        ];
    ...
    function drop() {
      for (let index = 0; index < 5; index++) {
        setTimeout(humanObjectAppear(), 500 * index);
      }
    }
